...
class data_manager(object):
    ...
    def get_single_instance(self,set_label,number):
        ...
        if self.channel==1:
            ...
            for idx in xrange(number):
                image[idx,0]=image_set[self.set_pt[set_label]]
                rd_idx1 = random.randint(0,image_set.shape[0]-1)
                image[idx,1] = image_set[rd_idx1]
                rd_idx2 = random.randint(0, image_set.shape[0] - 1)
                image[idx, 2] = image_set[rd_idx2]
                label[idx]=label_set[self.set_pt[set_label]]*100+label_set[rd_idx1]*10+label_set[rd_idx2]
                self.set_pt[set_label]+=1
                if self.set_pt[set_label]==number_of_instances:
                    end_of_epoch=True
                    self.set_pt[set_label]=0
